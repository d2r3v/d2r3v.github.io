...
export const projectsData: Project[] = [
  ...
  },
  {
    ...
    summary:
      "A full-stack system that persists conversational context, applies safety-aware routing, and structures execution for reliable AI interactions.",
    overview:
      "Most AI chat systems treat conversations as stateless. Reflect AI explores how persistent memory and structured execution pipelines enable more reliable and context-aware AI systems.",
    metrics: [
      "Implemented end-to-end authenticated chat flow with persistent conversation support",
      "Structured backend for context retrieval and future tool orchestration",
    ],
    contributions: [
    ...
    standoutSections: [
      {
        ...
        content: [
          "Separation of concerns between mobile client and backend services",
      ...
      },
      {
        ...
        content: [
          "Designing AI systems requires more than model calls — state, safety, and execution flow matter",
  ...
  },
  {
    ...
    tagline:
      "Backend system processing 1000+ student exam submissions with automated grading, analytics, and cross-variant cheating detection.",
    summary:
  ...
  },
  {
    ...
    summary:
      "Deep learning system identifying medications from real-world images (vision + OCR imprints), achieving 84% accuracy via gated modality fusion.",
    metrics: [
...
[Vision: MobileNetV2]    [OCR: CRNN]
 ...
 [Visual Features]      [Text Features]
    ...
    standoutSections: [
      {
        ...
        content:
          "Most pill identification systems rely on either image classification or text lookup, which breaks down in real-world conditions (missing text, similar pills). Instead of treating vision and OCR equally, PillCare employs a confidence gate that decides when OCR should influence predictions. This allows the system to ignore OCR when no readable text exists and use it when visual ambiguity is high. This approach raised accuracy to 84% (+3.6% over vision-only).",
      },
      {
        title: "Data and Class Imbalance Challenges",
        content:
          'Addressing data quality provided greater returns than model complexity. A 3x offline augmentation gave the most significant improvement (+9%). Furthermore, a severe class imbalance caused the model to ignore minority classes (e.g., amoxicillin). Utilizing `compute_class_weight("balanced")` combined with targeted augmentation successfully optimized cross-entropy loss across all 16 drug classes, guaranteeing non-zero recall for rare pills.',
      },
    ],
    tech: [
      "TensorFlow",
  ...
  },
  {
    ...
    tagline:
      "Unsupervised learning pipeline identifying engineering burnout entirely from behavioral telemetry",
    summary:
      "Data science pipeline analyzing temporal GitHub activity using Isolation Forests to detect early signals of developer burnout before critical turnover.",
    metrics: [
